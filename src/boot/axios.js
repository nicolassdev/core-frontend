...
const httpGet = (resource, callback, payload = {}) => {
	...
	newConfig.headers.Authorization = `Bearer ${LocalStorage.getItem('Bearer') || ''}`;
	ChainCallback(axios.get(resource, newConfig), callback);
};

const httpPost = (resource, payload, callback) => {
	// ALWAYS REBIND AUTHORIZATION HEADER TO REQUEST!
	axiosConfig.headers.Authorization = `Bearer ${LocalStorage.getItem('Bearer') || ''}`;
...
export { api, httpGet, httpPost, httpPut, httpDel, httpFileUpload, httpFileDownload };
